...
class BigtableTableAdminClient(object):
    ...
    def snapshot_table(
        ...
        Args:
            name (str): Required. The unique name of the table to have the snapshot taken.
                Values are of the form
                ``projects/{project}/instances/{instance}/tables/{table}``.
            cluster (str): Required. The name of the cluster where the snapshot will be created in.
                Values are of the form
                ``projects/{project}/instances/{instance}/clusters/{cluster}``.
            snapshot_id (str): Required. The ID by which the new snapshot should be referred to within
                the parent cluster, e.g., ``mysnapshot`` of the form:
    ...
    def create_backup(
        ...
        Args:
            parent (str): Required. This must be one of the clusters in the instance in which this
                table is located. The backup will be stored in this cluster. Values are
                of the form
                ``projects/{project}/instances/{instance}/clusters/{cluster}``.
            backup_id (str): Required. The id of the backup to be created. The ``backup_id`` along
                with the parent ``parent`` are combined as {parent}/backups/{backup\_id}
    ...
    def list_backups(
        ...
        Args:
            ...
            filter_ (str): A filter expression that filters backups listed in the response. The
                ...
                -  ``source_table:prod`` --> The source\_table's name contains the
                   string "prod".
                -  ``state:CREATING`` --> The backup is pending creation.
                -  ``state:READY`` --> The backup is fully created and ready for use.
                -  ``(name:howl) AND (start_time < \"2018-03-28T14:50:00Z\")`` --> The
                   backup name contains the string "howl" and start\_time of the backup
            ...
            order_by (str): An expression for specifying the sort order of the results of the
                ...
                Fields supported are: \* name \* source\_table \* expire\_time \*
                start\_time \* end\_time \* size\_bytes \* state

                For example, "start\_time". The default sorting order is ascending. To
                specify descending order for the field, a suffix " desc" should be
                appended to the field name. For example, "start\_time desc". Redundant
                space characters in the syntax are insigificant.

                If order\_by is empty, results will be sorted by ``start_time`` in
    ...
    def update_backup(
        ...
        Args:
            backup (Union[dict, ~google.cloud.bigtable_admin_v2.types.Backup]): Required. The backup to update. ``backup.name``, and the fields to be
                ...
                If a dict is provided, it must be of the same form as the protobuf
                message :class:`~google.cloud.bigtable_admin_v2.types.Backup`
            update_mask (Union[dict, ~google.cloud.bigtable_admin_v2.types.FieldMask]): Required. A mask specifying which fields (e.g. ``expire_time``) in the
    ...
    def restore_table(
        ...
        is ``RestoreTableMetadata``. The ``response`` type is ``Table``, if
        successful.
